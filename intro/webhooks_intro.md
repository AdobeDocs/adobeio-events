...
Your webhook must respond to the POST request with an HTTP status code of 200 before it can be put in the manual validation mode. In other words, if the webhook responds with a 200 but doesn't respond with a body containing the challenge, it is switched to a manual validation mode. If there is a GET request on the validation URL within 5 minutes, the webhook is marked `Active`.
